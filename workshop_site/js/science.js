(function(root,factory){const api=factory();if(typeof module!=="undefined"&&module.exports)module.exports=api;root.WorkshopScience=api})(typeof globalThis!=="undefined"?globalThis:this,function(){"use strict";
  ...
  function zhouDeterministic(theta,p0,generations=120){let p=p0.slice(),out=[p.slice()];const muTri=10**theta[0],muLoh=10**theta[3],fitness=[theta[1],1,theta[2]];for(let g=1;g<=generations;g++){const mutated=[p[0]*(1-muTri-muLoh),p[1]+p[0]*muTri,p[2]+p[0]*muLoh],selected=mutated.map((x,i)=>x*fitness[i]),total=selected.reduce((a,b)=>a+b,0);p=selected.map(x=>x/total);out.push(p.slice())}return out}
