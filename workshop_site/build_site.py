...
For posterior draw $m$, simulate lineage frequencies $f_i^{{(m)}}(t)$ and calculate the exponential
of Shannon entropy:

$$\theta^{{(m)}}\sim p(\theta\mid x_{{\mathrm{{obs}}}}),\qquad
f^{{(m)}}(t)\sim p(f\mid\theta^{{(m)}}),\qquad
D^{{(m)}}(t)=\exp\!\left[-\sum_i f_i^{{(m)}}(t)\log f_i^{{(m)}}(t)\right].$$

$D(t)$ is the effective number of equally abundant lineages. Repeating the calculation across
posterior draws produces a distribution over diversity rather than one deterministic curve.
