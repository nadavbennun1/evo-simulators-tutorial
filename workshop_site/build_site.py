...
def avecilla_model_builder_section() -> str:
    mutation = math_to_html(r'''<p class="force-number">01 · Mutation</p>
<h3>Mutation moves probability between states</h3>
<p>An ancestral cell can produce a descendant with a new, heritable state. Mutation supplies
variation; whether that variation later spreads is a separate question.</p>
<section class="equation-level scalar-level"><h4>Two genotypes</h4>
<p>Let $x_{A,t}$ and $x_{C,t}$ be the ancestral and mutant frequencies, with
$x_{A,t}+x_{C,t}=1$. If $A$ forms $C$ with probability $\delta$ and there is no reverse mutation,</p>
$$x_A^{(m)}=(1-\delta)x_{A,t},\qquad
x_C^{(m)}=x_{C,t}+\delta x_{A,t}.$$
<p>The term $\delta x_{A,t}$ is the new mutant frequency supplied from the ancestral population.</p></section>
<section class="equation-level vector-level"><h4>The same bookkeeping in matrix form</h4>
$$\begin{pmatrix}x_A^{(m)}\\x_C^{(m)}\end{pmatrix}=
\underbrace{\begin{pmatrix}1-\delta&0\\\delta&1\end{pmatrix}}_{M}
\begin{pmatrix}x_{A,t}\\x_{C,t}\end{pmatrix},\qquad x^{(m)}=Mx_t.$$
<p>Column $j$ of $M$ distributes descendants of parental genotype $j$ among their possible states;
therefore every column sums to one.</p></section>
...
<div class="symbol-chips"><span>$\delta_C$ · CNV formation</span><span>$\delta_B$ · other-beneficial formation</span></div>
    ...
    selection = math_to_html(r'''<p class="force-number">02 · Selection</p>
<h3>Selection reweights reproductive contribution</h3>
<p>Genotypes can differ in the number of descendants they are expected to contribute. Selection
changes the expected composition; it neither creates the mutant nor eliminates stochasticity.</p>
<section class="equation-level scalar-level"><h4>Two genotypes</h4>
<p>Take the ancestor as the fitness reference, $w_A=1$, and give the mutant fitness $w_C=1+s$.
After mutation, their unnormalized contributions are $x_A^{(m)}$ and $(1+s)x_C^{(m)}$.
Dividing the mutant contribution by the total gives</p>
$$x_C^{(s)}=\frac{(1+s)x_C^{(m)}}{x_A^{(m)}+(1+s)x_C^{(m)}},\qquad
x_A^{(s)}=1-x_C^{(s)}.$$
...
with the other beneficial state.</p>
    ...
    drift = math_to_html(r'''<p class="force-number">03 · Drift</p>
<h3>Drift turns an expectation into one realized population</h3>
<p>Only a finite number of descendants establish the next modeled generation. Chance affects which
descendants are represented, so identical starting populations can follow different trajectories.</p>
<section class="equation-level scalar-level"><h4>Two genotypes</h4>
<p>Once mutation and selection give the expected mutant frequency $x_C^{(s)}$, the realized mutant
count is a binomial draw:</p>
$$n_{C,t+1}\sim\operatorname{Binomial}(N_e,x_C^{(s)}),\qquad
x_{C,t+1}=\frac{n_{C,t+1}}{N_e},\qquad n_{A,t+1}=N_e-n_{C,t+1}.$$
<p>The expectation is still $x_C^{(s)}$, but the realized value need not be exactly equal to it.</p></section>
<section class="equation-level vector-level"><h4>More than two genotypes</h4>
<p>A multinomial draw is the vector version of the same finite-population process:</p>
$$n_{t+1}\sim\operatorname{Multinomial}(N_e,x^{(s)}),\qquad
x_{t+1}=\frac{n_{t+1}}{N_e}.$$
<p>All genotype counts are drawn together and must sum to $N_e$.</p></section>
<section class="equation-level paper-level"><h4>Avecilla: three genotypes</h4>
$$\bigl(n_A,n_C,n_B\bigr)_{t+1}\sim
\operatorname{Multinomial}\!\left(N_e,\bigl(x_A^{(s)},x_C^{(s)},x_B^{(s)}\bigr)\right).$$
<p>Smaller $N_e$ creates more variation among replicate trajectories. $N_e$ is the population size
that reproduces the strength of drift; it need not equal the physical number of cells in the vessel.</p>
</section>''')
    ...
    migration_img = versioned_asset('assets/chapter/migration-cartoon.jpg')
    mating_img = versioned_asset('assets/chapter/assortative-mating-cartoon.jpg')
    complete = math_to_html(r'''<p>Combined in the workshop convention:</p>
$$x_t\xrightarrow{\text{mutation }M}x^{(m)}\xrightarrow{\text{selection }W}x^{(s)}
\xrightarrow{\text{finite-population drift}}x_{t+1}.$$''')
    return f'''<section class="lesson-cell model-builder" id="avecilla-model-builder">
      <p class="section-kicker">Modeling evolutionary forces</p><h2>From two genotypes to the Avecilla model</h2>
      <p class="model-builder-intro">Mutation, selection, and drift play different roles in one generation. Each force is easiest to see first with an ancestor and one mutant; the same bookkeeping then extends to the three genotypes in the experiment.</p>
      <div class="force-lessons">
        <article class="force-lesson"><figure><img src="{mutation_img}" alt="A cell changes from the ancestral state to a new heritable state"><figcaption>Mutation supplies a new heritable state.</figcaption></figure><div class="force-lesson-copy">{mutation}</div></article>
        <article class="force-lesson"><figure><img src="{selection_img}" alt="A fitter green cell leaves more descendants than an orange cell"><figcaption>Selection changes expected reproductive contribution.</figcaption></figure><div class="force-lesson-copy">{selection}</div></article>
        <article class="force-lesson"><figure><img src="{drift_img}" alt="A chance bottleneck changes the colors represented in a finite population"><figcaption>Drift makes finite-population histories stochastic.</figcaption></figure><div class="force-lesson-copy">{drift}</div></article>
      </div>
      <div class="complete-generation">{complete}</div>
      <aside class="broader-forces">
        <div class="broader-forces-heading"><p class="section-kicker">The theory is broader</p><h3>Two central forces are absent from this experimental model</h3><p>Population-genetic theory also describes movement among populations and non-random mating. The workshop simulators omit these forces because the experiments modeled here use isolated replicate cultures propagated predominantly by clonal division.</p></div>
        <article>
          <img src="{migration_img}" alt="Cartoon cells moving between two populations in a cycle">
        ...
        <article>
          <img src="{mating_img}" alt="Cartoon cells preferentially pairing with cells of the same color">
...
The interactive view uses one representative parameter set. The published result repeats the
...
1.6 × 10<sup>4</sup> lineages for ARSΔ to 3.2 × 10<sup>5</sup> for wild type. The ordering follows the
