...
\begin{abstract}
Experimental evolution produces replicated, stochastic time-series data that
are rich in biological information but difficult to analyse with classical
likelihood-based methods.
This paper is a practical tutorial for researchers who want to use
simulation-based inference~(SBI) to estimate mutation rates, selection
coefficients, and related evolutionary parameters from such data.
Using copy-number-variant~(CNV) and aneuploidy dynamics in
\emph{Saccharomyces cerevisiae} as running examples, we explain the full
workflow from simulator design to posterior interpretation.

The tutorial has three goals.
First, it introduces the modelling side of the problem through simple
stochastic population-genetic simulators, beginning with Wright-Fisher
models and their chemostat analogue.
Second, it compares approximate Bayesian computation~(ABC) with neural SBI
methods such as neural posterior estimation~(NPE), emphasizing when each
approach is useful in practice.
Third, it develops the \emph{collective posterior}, a principled way to
combine evidence across multiple independent experimental replicates.

The paper is designed to be read either sequentially as a tutorial or
selectively as a reference.
Readers interested mainly in practice can move directly from the problem
formulation to the applications, while readers interested in methodology can
focus on the SBI and collective-posterior sections.
Each major section is paired with accompanying Jupyter notebooks that serve
as the executable supplement to the paper.
\end{abstract}
...
This paper is intentionally written as a tutorial rather than as a narrow
methods note.
Our aim is not only to present a set of inference results, but to give the
reader a reusable way of thinking about experimental-evolution data:
start from a mechanistic simulator, define a prior over biologically
meaningful parameters, train or run an SBI method, and then interpret the
resulting posterior across one or many replicates.

\subsection{What This Paper Is}

At a high level, this paper does three things.
It explains the inference problem posed by stochastic evolutionary
trajectories, it introduces a small family of forward simulators that are
simple enough to understand and fast enough for SBI, and it shows how to
move from single-replicate inference to principled multi-replicate
inference.
The emphasis throughout is practical: we focus on models and workflows that
a reader can adapt to new experimental systems with modest changes.

The paper is therefore best viewed as a guide to a workflow, not just as a
catalogue of results.
The figures and applications illustrate concrete biological systems, but the
main object being taught is the pipeline itself.

\subsection{How To Read This Tutorial}

There are several reasonable ways to navigate the paper.

\begin{itemize}
  \item \textbf{Sequential tutorial read.} Read Sections~\ref{sec:problem},
        \ref{sec:simulators}, \ref{sec:sbi}, \ref{sec:collective}, and
        \ref{sec:applications} in order.
        This path is intended for readers who are new to SBI in
        experimental evolution.
  \item \textbf{Methods-focused read.} If you already know the biological
        systems, focus on Sections~\ref{sec:sbi} and \ref{sec:collective},
        then use Section~\ref{sec:applications} as a worked example library.
  \item \textbf{Application-first read.} If you want to see what the full
        workflow looks like before reading the details, skim
        Section~\ref{sec:applications} first and then return to the earlier
        sections as needed.
\end{itemize}

The Jupyter notebooks mirror this structure.
\texttt{SBI\_tutorial.ipynb} is the end-to-end worked example for the
Chuong et al.\ system, while \texttt{evolution\_simulators.ipynb} serves as
a companion atlas of the simulator family used throughout the paper.

The tutorial has four main contributions:
\begin{enumerate}
  \item We describe and compare ABC and neural SBI methods (NPE, NLE, and
        NRE) in the context of experimental evolution, providing guidance on
        when each approach is preferred.
  \item We present Wright-Fisher and chemostat ODE simulators for four
        biologically distinct experimental systems, demonstrating how the same
        inference framework adapts to different model structures
        (Figure~\ref{fig:four_simulators}).
  \item We derive and implement the \emph{collective posterior}, a
        principled formula for aggregating evidence from multiple independent
        replicates---the regime that is standard in experimental evolution but
        often ignored by off-the-shelf SBI tools.
  \item We provide fully reproducible Jupyter notebooks that accompany
...
% =============================================================================
\section{The Collective Posterior: Combining Evidence Across Replicates}
...
We illustrate the complete inference pipeline on four experimental systems.
In each case we (i) describe the biological system and data, (ii) specify
the simulator and prior, (iii) train an NPE, (iv) compute individual and
collective posteriors, and (v) perform a posterior predictive check.
Figure~\ref{fig:four_simulators} provides an overview of all four systems
and their model fits.
...
\subsection{Choosing a method and simulator}
