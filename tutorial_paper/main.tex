...
Second, budget-fair ensemble strategies alter the
accuracy-calibration-cost tradeoff, sometimes recovering part of the benefit
of longer training but not uniformly outperforming a single fit.
...
\begin{enumerate}
  ...
  \item \textbf{Ensemble gains are system-dependent.}
        Budget-fair ensembles can recover part of the benefit of longer
        training in some systems, but they are not a free lunch:
        calibration, posterior width, and wall-clock cost move differently
        across benchmarks.
...
For completeness, we also include an appendix-style closing section on the
collective posterior for combining evidence across replicates
(Section~\ref{sec:collective}).
That topic belongs to a separate companion methods paper and is placed at
the end so it supports the main reliability story rather than competing with
it.
...
We therefore begin with three controlled benchmark analyses drawn from the
saved tutorial notebooks and then turn to four empirical systems.
Across both settings we use posterior shape, held-out recovery, and
posterior-predictive fit to study three recurring issues:
how strongly the posterior depends on the prior, how stable inference is
across repeated neural fits, and how visible model misspecification becomes
once one checks simulated trajectories against data.

\subsection{Controlled reliability benchmarks}
\label{sec:reliability_benchmarks}

Before turning to the empirical datasets, we summarise the three benchmark
results that motivate the rest of the paper.
These analyses come directly from the saved outputs in
\texttt{sbi\_paper\_experiments}, and they establish the three main findings
claimed in the Introduction.

\subsubsection{Prior support shifts the posterior even under a matched simulator}

We first revisit the dedicated prior-sensitivity experiments in
\texttt{sbi\_paper\_experiments}, which compare a lower and higher prior on
three benchmark systems (Avecilla et al., Zhou et al., and Vande Zande et al.) at both
10,000 and 100,000 simulations.
Figure~\ref{fig:prior_sensitivity} shows a consistent pattern in the core
recovery metrics:
across all six model-budget combinations, the higher prior yields lower NMAE
and narrower credible intervals.
For example, in the Avecilla et al.\ benchmark at 100,000 simulations the NMAE falls
from 0.054 to 0.040 and the normalised interval width from 0.252 to 0.155;
in Zhou et al.\ it falls from 0.087 to 0.065 with the interval width shrinking from
0.326 to 0.239; and even in the harder Vande Zande et al.\ setting the higher prior
improves NMAE from 0.112 to 0.103 while reducing the interval width from
0.494 to 0.435.
At the same time, calibration and predictive checks are more mixed.
The higher prior improves SBCC markedly in some settings, such as Avecilla et al.\ at
10,000 simulations (0.054 to 0.012) and Vande Zande et al.\ at 10,000 simulations
(0.103 to 0.029), but in other cases it worsens predictive RMSE or SBCC.
The point is therefore broader than ``pick the narrow prior'':
prior design changes both recovery and uncertainty structure, and those
changes are scientifically consequential.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{fig_prior_sensitivity.png}
  \caption{\textbf{Prior sensitivity across three benchmark systems.}
  Paired points compare lower and higher priors for the Avecilla et al.,
  Zhou et al., and Vande Zande et al.\ benchmarks at 10,000 and
  100,000 simulations.
  Moving from the lower to the higher prior reduces NMAE and posterior width
  in all six cases, but the calibration and predictive-fit metrics are more
  heterogeneous.
  This is the core methodological result of the prior experiments:
  prior choice changes the inference answer, not just the aesthetics of the
  posterior.
  The plot was generated from the saved summaries in
  \texttt{sbi\_paper\_experiments/results/prior}.}
  \label{fig:prior_sensitivity}
\end{figure}

\subsubsection{Budget-fair ensembles change the accuracy-calibration-cost tradeoff}

The ensemble experiments are explicitly budget-fair:
patience 30 is compared against a prolonged single run with patience 60 and
against $K=3$ and $K=5$ vanilla ensembles that divide the original budget
across members.
Figure~\ref{fig:ensemble_comparison} shows that these experiments do not
support a blanket claim that ensembles always improve inference.
Instead, they reveal a system-dependent tradeoff.
In the Moshe et al.\ benchmark, prolonged training gives the best calibration
(SBCC 0.014) and best NMAE (0.237), but the ensembles recover most of that
accuracy at substantially lower wall-clock cost:
the $K=3$ ensemble reaches NMAE 0.239 in 39 minutes, compared with 103 minutes
for the prolonged run.
In the Avecilla et al.\ benchmark, $K=5$ marginally improves NMAE
(0.0552 versus 0.0557 for a single run) but does so by substantially widening
the posterior and worsening SBCC (0.124 versus 0.016).
In Vande Zande et al., the single model remains the best-calibrated and fastest
option.
The ensemble finding is therefore methodological rather than purely
optimistic:
ensembling changes the bias-calibration-cost tradeoff, and whether that is an
improvement depends on the scientific objective.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{fig_ensemble_comparison.png}
  \caption{\textbf{Budget-fair comparison of single, prolonged, and ensemble
  training.}
  Bars compare four training strategies across three benchmark systems:
  a single run at the base budget, a prolonged single run, and vanilla
  ensembles with $K=3$ and $K=5$.
  The results are mixed by design.
  Ensembles can recover much of the benefit of longer training in some
  systems, especially Moshe et al., but can also worsen calibration or widen
  the posterior, as in the Avecilla et al.\ and Vande Zande et al.\ benchmarks.
  The figure was generated from the saved experiment metadata and evaluation
  summaries in \texttt{sbi\_paper\_experiments/results/ensembles}.}
  \label{fig:ensemble_comparison}
\end{figure}

\subsubsection{Misspecification is clearest in the Avecilla et al.\ versus Chuong et al.\ use case}

Finally, we restrict misspecification to the intended empirical case:
the rise phase of the Chuong et al.\ LTR$\Delta$ trajectories is truncated to
generations 8--66, and that same window is analysed with either the correct
Chuong et al.\ model or the misspecified Avecilla et al.\ model.
Figure~\ref{fig:misspecification_metrics} makes this trap visible directly in
the data.
Within the shaded fit window, both models can track the rise phase
reasonably well.
Once the same posterior samples are rolled forward past generation 66,
however, the difference becomes obvious: the misspecified Avecilla et al.\
model systematically overshoots or misses the later plateau dynamics,
whereas the Chuong et al.\ model remains much closer to the observed
trajectories.
The quantitative summaries tell the same story in a more compact form.
First, the posterior means shift much more strongly for mutation rate than
for selection, so the wrong model changes the inferred biology rather than
simply adding extra uncertainty.
Second, the truncated-window fit itself is deceptively permissive.
The mean RMSE on generations 8--66 is only 0.048 for the misspecified
Avecilla et al.\ model versus 0.021 for the correct Chuong et al.\ model, so both can appear
acceptable if one stops at the observed window.
Once the same posteriors are rolled forward to the full trajectories through
generation 116, however, the discrepancy becomes obvious:
the mean RMSE is 0.211 for the Avecilla et al.\ model and 0.032 for the Chuong et al.\ model.
This is exactly the kind of misspecification trap experimental evolution is
vulnerable to:
a biologically nearby simulator can fit the rise phase while still failing on
the plateau dynamics that matter mechanistically.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{fig_misspecification_metrics.png}
  \caption{\textbf{Misspecification is only obvious after forward prediction.}
  Each column shows one Chuong et al.\ LTR$\Delta$ replicate.
  Black points and lines are the observed data.
  Colored bands and median lines are posterior predictive trajectories from
  the misspecified Avecilla et al.\ model (top row) or the correct Chuong et al.\ model
  (bottom row).
  The shaded region marks the truncated fit window (generations 8--66), where
  both models can appear acceptable.
  Once predictions are rolled forward beyond the dashed boundary at
  generation~66, the misspecified Avecilla et al.\ model fails much more
  clearly on the later plateau dynamics.
  The figure follows the exact Avecilla et al.-versus-Chuong et al.\ analysis in
  \texttt{sbi\_paper\_experiments/notes/misspecification\_results.ipynb}.}
  \label{fig:misspecification_metrics}
\end{figure}

\subsection{Empirical applications}
\label{sec:empirical_applications}

With those benchmark findings in hand, we now evaluate the SBI workflow across
...
\subsubsection{Optional multi-replicate aggregation}

The ALLΔ system is also a natural place to demonstrate how one can combine
evidence across replicate posteriors once the single-replicate model has been
validated.
Because that construction is part of a separate companion methods paper, we
place the derivation and the ALLΔ collective-posterior example in the
closing appendix-style section (Section~\ref{sec:collective}) rather than in
the main results flow here.
...
cytometry data.
Third, multi-replicate aggregation methods such as the collective posterior
summarised in Section~\ref{sec:collective} assume that all replicates share
the same $\btheta$; in practice, there may be biologically meaningful
between-replicate parameter variation that a single shared posterior cannot
capture.
...
The general framework---forward simulator, NPE training, and optional
multi-replicate aggregation---is modular and can accommodate each of these extensions with
minimal changes to the inference code.

% =============================================================================
\section{Appendix: Multi-Replicate Inference}
\label{sec:collective}
% =============================================================================

The material in this closing section summarises one important extension of
the single-replicate SBI workflow developed in the main paper.
Because multi-replicate aggregation is a substantial topic in its own right,
and because it is the focus of separate companion work, we place it here as
supporting methodology rather than as one of the main findings of the
present manuscript.
...
\subsection{ALLΔ example: collective posterior and posterior predictive check}
