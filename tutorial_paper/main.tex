...
%                     model schematics are one level up in the repo root.
% Overleaf:           copy avecilla_wf_model.png, chuong_wf_model.png,
%                     de_et_al_model.png, zhou_model.png into the Overleaf
%                     project root and change the second path below to {./}
\graphicspath{{./}{../}}
...
\begin{abstract}
Experimental evolution provides time-resolved, replicated measurements of
evolving populations, offering a unique window into fundamental evolutionary
processes such as de~novo mutation, selection, and genetic drift.
A central challenge is to extract quantitative estimates of these processes
from stochastic population-genetic data whose likelihoods are analytically
intractable.
Simulation-based inference~(SBI) bypasses this obstacle by replacing
likelihood evaluation with a comparison between simulated and observed data,
either through rejection sampling or by learning a parametric approximation
of the posterior distribution from simulations.
Here we present a practical tutorial on applying SBI to experimental evolution
data, using copy-number-variant~(CNV) and aneuploidy dynamics in
\emph{Saccharomyces cerevisiae} as running examples.
We begin with the Wright-Fisher model and its chemostat analogue, then
compare approximate Bayesian computation~(ABC) with neural posterior
estimation~(NPE) and related neural approaches.
We pay particular attention to the multi-replicate setting that is standard
in experimental evolution, and derive the \emph{collective posterior}---a
principled formula for combining evidence across independent replicates.
All methods are implemented in self-contained Jupyter notebooks that
constitute the online supplement to this paper.
\end{abstract}
...
This tutorial has four main contributions:
\begin{enumerate}
